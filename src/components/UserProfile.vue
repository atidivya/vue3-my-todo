<template>
  <div class="user-profile">
    <div class="user-profile_user-panel">
      ...
      <form class="user-profile_create-todo" @submit.prevent="createNewTodo" :class="{ '--exceeded': newTodoCharacterCount > 180}">
        <label for="newTodo"><strong>New Todo</strong>({{ newTodoCharacterCount}}/180)</label>
        <textarea
          name=""
          id="newTodo"
          cols="30"
        ...
        <div class="user-profile_create-todo-type">
          ...
          <select id="newTodoType" v-model="selectedTodoType">
            <option
              :value="option.value"
              v-for="(option, index) in todoTypes"
              :key="index"
            >
              {{option.name}}
            </option>
...
export default {
  ...
  computed: {
    newTodoCharacterCount() {
      return this.newTodoContent.length;
    }
  },
  methods:{
    followUser(){
